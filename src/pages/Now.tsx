...
function NowMedia({ image }: { image: NowImage }) {
  const ratio =
    image.aspect === 'portrait' ? 'aspect-[4/5]' : image.aspect === 'square' ? 'aspect-square' : 'aspect-[16/10]';
  return (
  ...
  return (
    <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
      <h4 className="flex items-center gap-2 text-lg font-bold tracking-tight text-ink sm:text-xl">
        {entry.active && <span aria-hidden="true" className="h-2 w-2 shrink-0 rounded-full bg-red" />}
        {entry.title}
      </h4>
  ...
  return (
    <main className="relative" style={{ zIndex: 'var(--z-content)' }}>
      <Section id="now" className="flex flex-col gap-16">
        <SheetHeading title="Now" meta={`Updated ${updatedLabel}`} description={nowIntro} />
